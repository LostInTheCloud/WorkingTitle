...
void SDL_Display::display_draw(const uint32_t* pixels)
{
    ...
    SDL_Surface* native_surface = SDL_CreateRGBSurfaceFrom((void*) pixels, (int) _width, (int) _height, 4 * 8,
                                                           4 * (int) _width, Rmask, Gmask, Bmask, Amask);
...
#define SDL_EVENT_CASE(x) case x: fprintf(stderr, "%s", #x);
        ...
        fprintf(stderr, "(THIS IS PROBABLY A BUG!)");
        break;
        SDL_EVENT_CASE(SDL_QUIT)
        fprintf(stderr, "(timestamp=%u)", (uint) event->quit.timestamp);
        break;
...
#define SDL_EVENT_CASE(x) case x: fprintf(stderr, "%s ", #x);
        ...
        fprintf(stderr, "(timestamp=%u windowid=%u event=", (uint) event->window.timestamp,
                (uint) event->window.windowID);
        switch(event->window.event)
        {
            case SDL_WINDOWEVENT_NONE: fprintf(stderr, "none(THIS IS PROBABLY A BUG!)");
                break;
...
#undef SDL_WINDOWEVENT_CASE
            default: fprintf(stderr, "UNKNOWN(bug? fixme?)");
                break;
        }
...
#undef PRINT_KEY_EVENT
        ...
        fprintf(stderr, "(timestamp=%u windowid=%u text='%s')", (uint) event->text.timestamp,
...
#undef SDL_EVENT_CASE
        ...
        default:fprintf(stderr, "UNKNOWN SDL EVENT #%u! (Bug? FIXME?)", (uint) event->type);
            break;
    }
